...
setwd("S:/ra/RADUMP/Chief Economist/R Projects/ASU Project with Ohio")

# Set Variables ------------------
State_FIPS <- 09
Census_File <- "CT_asu25.xlsx"
UR_ASU_Threshold <- 6.45

# Download TIGER/Line shapefile from US Census Bureau ------------------
State_Tracts <- tracts(State_FIPS, year = 2021)
...
new_included <- new_data %>%
  select("GEOID", "name", "tract_pop2023", "tract_ASU_clf", "tract_ASU_emp", "tract_ASU_unemp", "tract_ASU_urate","asunum") %>%
  filter(asunum != 0)
